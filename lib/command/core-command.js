...
CoreCommand.prototype.getVersion = function() {
  ...
  let packet = new CommandPacket(CORE_API.DID_CORE, CORE_API.CMD_VERSION);

  return this.send(CONTROL_SERVICE.id, CONTROL_SERVICE.commandsCharacteristic, packet.packetBuffer);
};
...
CoreCommand.prototype.setBluetoothName = function(name) {
  ...
  let packet = new CommandPacket(CORE_API.DID_CORE, CORE_API.CMD_SET_BT_NAME, data);
...
CoreCommand.prototype.getBluetoothInfo = function() {
  ...
  let packet = new CommandPacket(CORE_API.DID_CORE, CORE_API.CMD_GET_BT_NAME);
...
CoreCommand.prototype.setAutoReconnect = function(enable, time) {
  ...
  if (enable && time === undefined) {
    time = 0x30;
  } else if (!enable && time === undefined) {
    time = 0x00;
  }
  ...
  data.writeUInt8(enable, 0x00);
  data.writeUInt8(time, 0x01);

  let packet = new CommandPacket(CORE_API.DID_CORE, CORE_API.CMD_SET_AUTO_RECONNECT, data);
...
CoreCommand.prototype.getAutoReconnect = function() {
  ...
  let packet = new CommandPacket(CORE_API.DID_CORE, CORE_API.CMD_GET_AUTO_RECONNECT);

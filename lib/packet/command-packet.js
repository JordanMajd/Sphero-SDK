 ...
 * @api public
 */
function CommandPacket(_did, _cid, _data, _seq, requestAnswer, resetTimeout) {
  ...
  this.cid = _cid || 0x00;
  this.seq = _seq || 0x00;
  this.data = _data || new Buffer(0);
  this.dlen = this.data.length + 0x01;

  // SOP1 SOP2 DID CID SEQ DLEN CHK
  this.packetBuffer = new Buffer(this.data.length + 0x07);
  ...
  this.chck = this.computeCheckSum(this.packetBuffer.slice(0, this.packetBuffer.length - 0x01));
